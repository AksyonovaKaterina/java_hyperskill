...
import com.bad_java.homework.hyperskill.tictactoe_with_al.part_4.StateOfBoard.*;
...
public class Board {
    ...
    public int whereToStep(){
        for (POSSIBLE_LINES line : POSSIBLE_LINES.values()) {
            char firstSym = board.get(line.getFirst()).getSymbol();
            char secondSym = board.get(line.getSecond()).getSymbol();
            char thirdSym = board.get(line.getThird()).getSymbol();
            int whichIsEmpty = whichIsEmpty(firstSym, secondSym, thirdSym);
            if (whichIsEmpty != -1) {
                switch (whichIsEmpty) {
                    case 1:
    ...
    private int whichIsEmpty(char firstSym, char secondSym, char thirdSym){
        if (empty(firstSym, secondSym, thirdSym)){
            return 1;
        }
        if (empty(secondSym, firstSym, thirdSym)){
            return 2;
        }
        if (empty(thirdSym, secondSym, firstSym)){
            return 3;
        }
        return -1;
    }

    private boolean empty(char one, char two, char three) {
